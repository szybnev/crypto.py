...
d_text = f.renderText('Decrypt')
e_text = f.renderText('Encrypt')
bb_text = f.renderText('Bye')
...
space_count = 30 * ' '
cprint('{} File Encryption And Decryption Tool. {}'.format(space_count,space_count), 'green')
cprint('{} {}'.format(space_count + 3 * ' ','Programmed by Sergey Zybnev.'),'green')
while True:
		cprint('1. Encryption',color='magenta')
		cprint('2. Decryption',color='magenta')
		cprint('3. Exit', color='red')
		cprint('Input:',end=' ', color='green')
		choice = int(input())

		if choice == 1:
			cprint(e_text,color='green')
			...
			cprint('Back to menu (y/n):',end = ' ', color='red',attrs=['bold','blink'])
			again_choice  = input()
